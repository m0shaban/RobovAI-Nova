...
class QuickChartTool(BaseTool):
    ...
    def description(self):
        return "إنشاء رسوم بيانية (bar, line, pie, doughnut)"
...
**الأنواع المتاحة:** bar, line, pie, doughnut, radar""",
        ...
        for i, part in enumerate(parts[1:]):
            if ":" in part:
                ...
                if chart_type in ["pie", "doughnut", "polarArea"]:
                    ...
                    labels.append(name)
                    if not datasets:
                        datasets.append({"data": [], "backgroundColor": colors})
        ...
        chart_url = (
            f"https://quickchart.io/chart?c={encoded}&backgroundColor=rgb(20,20,25)"
        )
...
class MathSolverTool(BaseTool):
    ...
    @property
    def description(self):
        return "Solve math expressions. Pass the expression in 'query', e.g. query='5+5' or query='sqrt(144)'"
...
class TextAnalyzerTool(BaseTool):
    ...
    @property
    def description(self):
        return "تحليل نص (كلمات، أحرف، جمل، قراءة)"
...
class TextCaseTool(BaseTool):
    ...
    @property
    def description(self):
        return "تحويل حالة النص (upper, lower, title, reverse)"
...
class PasswordStrengthTool(BaseTool):
    ...
    @property
    def description(self):
        return "تحليل قوة كلمة المرور وتقديم نصائح"
...
class DateCalculatorTool(BaseTool):
    ...
    @property
    def description(self):
        return "حساب الفرق بين تاريخين أو إضافة أيام"
...
class RandomPickerTool(BaseTool):
    ...
    @property
    def description(self):
        return "اختيار عشوائي من قائمة أو رقم عشوائي"
...
`/pick coin`""",
        ...
        if text.lower().startswith("dice"):
            ...
            dice_emoji = {1: "⚀", 2: "⚁", 3: "⚂", 4: "⚃", 5: "⚄", 6: "⚅"}.get(
                result, "🎲"
            )
            return {
                "status": "success",
                "output": f"🎲 **نرد {sides} أوجه:**\n\n{dice_emoji} **{result}**",
                "tokens_deducted": self.cost,
            }
        ...
        if items:
            ...
            return {
                "status": "success",
                "output": f"🎲 **الاختيار من {len(items)} عناصر:**\n\n✨ **{result}**",
                "tokens_deducted": self.cost,
            }
...
class UnitConverterTool(BaseTool):
    ...
    @property
    def description(self):
        return "تحويل الوحدات (طول، وزن، حرارة، مساحة)"
...
- **حرارة:** C, F, K""",
        ...
        # Parse: "100 km to mi"
        match = re.match(
            r"([\d.]+)\s*(\w+)\s*(?:to|إلى)\s*(\w+)", user_input.strip(), re.IGNORECASE
        )
        if not match:
            return {
                "status": "error",
                "output": "❌ صيغة غير صحيحة. مثال: `/convert 100 km to mi`",
                "tokens_deducted": 0,
            }
        ...
        return {
            "status": "error",
            "output": f"❌ وحدة غير معروفة: {from_unit} أو {to_unit}",
            "tokens_deducted": 0,
        }
...
class DiagramTool(BaseTool):
    ...
    @property
    def description(self):
        return "إنشاء مخططات (flowchart, sequence, class, pie)"
