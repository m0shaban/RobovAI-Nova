...
class FileCreatorInput(BaseModel):
    ...
    filename: str = Field(
        description="File name with extension, e.g. 'page.html', 'style.css', 'data.json'"
    )
    content: str = Field(description="The full file content to write")
    query: Optional[str] = Field(
        default=None, description="Optional description (not used)"
    )
...
class FileCreatorTool(BaseTool):
    ...
    async def execute(self, user_input: str, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "output": f"❌ خطأ في إنشاء الملف: {str(e)}"}
