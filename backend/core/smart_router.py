...
class SmartToolRouter:
    ...
    TOOL_PATTERNS = {
        ...
        '/weather': {
            ...
            'extract': lambda m: {'location': m.group(2).strip() if m.lastindex >= 2 and m.group(2) else 'Cairo'},
            'confidence': 0.95
        },
        ...
        '/convert': {
            ...
            'extract': lambda m: {'value': m.group(2), 'from': m.group(3), 'to': m.group(4)} if m.lastindex >= 4 else {},
            'confidence': 0.9
        },
    ...
    def get_context(cls, user_id: str, platform: str = "web") -> ConversationContext:
        ...
        if user_id not in cls._contexts:
            cls._contexts[user_id] = ConversationContext(user_id=user_id, platform=platform)
    ...
    def check_rate_limit(cls, user_id: str) -> bool:
        ...
        # Clean old entries
        cls._rate_limits[user_id] = [t for t in cls._rate_limits[user_id] if t > minute_ago]
    ...
    @classmethod
    async def detect_tool_semantic(cls, message: str, context: ConversationContext) -> Optional[RoutingResult]:
        """
...
"""
    ...
    @classmethod
    async def detect_tool(cls, user_message: str, user_id: str = "default", platform: str = "web") -> Optional[str]:
        """
    ...
    @classmethod
    async def route(cls, message: str, user_id: str = "default", platform: str = "web") -> RoutingResult:
        """
    ...
    @classmethod
    async def route_message(cls, user_message: str, user_id: str, platform: str = "web") -> Dict[str, Any]:
        """
        ...
        if result.route_type == 'tool' and result.tool_name:
            ...
            if tool_class:
                ...
                try:
                    ...
                    # Track usage
                    context.tool_usage[result.tool_name] = context.tool_usage.get(result.tool_name, 0) + 1
    ...
    def get_user_stats(cls, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_used_tool': max(context.tool_usage, key=context.tool_usage.get) if context.tool_usage else None,
    ...
    def cleanup_old_contexts(cls, max_age_hours: int = 24):
        ...
        to_remove = [uid for uid, ctx in cls._contexts.items() if ctx.last_active < cutoff]
